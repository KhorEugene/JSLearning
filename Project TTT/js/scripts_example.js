...
// Execute game loop to play in console
// Run this script using "node .\js\scripts_example.js"
...
async function askMultipleQuestions() {
  while (true){
    let column = await askQuestion(`${game.getActivePlayer().name}, enter column (0-6):`);
    console.log(`Answer 1: ${column}`);
    // Validate input:
    if (column === null) { // Allow the user to exit by cancelling the prompt
      break; // Exit the loop
    }
  ...
  console.log("Game Over!"); // Message printed when the game loop is exited
